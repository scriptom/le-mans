...
BEGIN
	...
	select f.ano into ano
	from h_participacion as pa, d_fecha as f
	...
	and f.id=(
		select faux.id
		from d_fecha as faux, h_participacion as paux
...
CREATE OR REPLACE FUNCTION public.reporte_diez_data(inp_ano integer)
    RETURNS TABLE(piloto_nombre varchar, piloto_pais varchar, piloto_foto_pais varchar, piloto_foto varchar, out_ano integer) 
    LANGUAGE 'plpgsql'
    COST 100
			...
			LOOP
 			IF inp_ano IS NULL THEN
				...
				IF ano IS NOT NULL THEN
 					FOR var_z IN (
 						select 
 						pi1.nombre as piloto_nombre1, pi1.pais_origen as piloto_pais1, encode(pi1.foto_pais::bytea, 'hex') as piloto_foto_pais1,
